...
static int pwm_led_esp32_timer_config(struct pwm_ledc_esp32_channel_config *channel)
{
	...
	/**
	 * ESP32 S2,S3 and C3 has only 14 bits counter.
	 * where as the plain ESP32 variant has 20 bits counter.
	 * application failed to set very low frequency(1Hz) in S2, S3 and C3 variants.
	 ...
	 * this will be handled on the function 'pwm_led_esp32_timer_set' with 'prescaler'.
	 */
	channel->clock_src = LEDC_REF_TICK;
